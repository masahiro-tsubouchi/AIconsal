...
- Frontend（`frontend/src/hooks/useWebSocket.ts` など）
  - 追加: `type === "debug_event"` の受信ブランチ。開発者用の最小デバッグパネル（`components/debug/DebugPanel.tsx`）に表示し、`onDebugEvent` でリアルタイム可視化（コンソールログは維持）。
...
- 受け入れ基準
  ...
  - サニタイズ方針（詳細）:
    - 除外（トップレベル優先、将来的に再帰的除去も想定）: `state`, `input`, `inputs`, `context`, `config`, `output`, `chunk` など。
    - 長文トリム: 文字列は最大500文字に短縮（設定で調整可能）。
  - 環境変数（Streaming関連）:
    - Backend: `DEBUG_STREAMING`（既定 `false`）, `DEBUG_BREAKPOINTS`（既定 `false`）, `ENABLE_CHECKPOINTER`（任意）。
    - Frontend: `REACT_APP_DEBUG_STREAMING`（既定 `false`、`true` で `?debug_streaming=1` を自動付与）。
  - UI要件（最小）:
    - Chatヘッダーの `Activity` アイコントグルで DebugPanel を表示/非表示できる。
  - 非対象（据え置き）:
    - トークンストリーミング（LLM出力の逐次トークン配信）は本計画では Deferred（仕様検討の上、別フェーズで導入）。
...
- ログ/イベントは開発者向け機能（フラグON時）に限定し、既定はOFF。
 - 本番運用ガードレール:
   - `DEBUG_STREAMING=false` / `DEBUG_BREAKPOINTS=false` を厳守（compose/envで固定）。
   - フロントも `REACT_APP_DEBUG_STREAMING=false` を既定とし、本番ビルドでは有効化しない。
   - 必要に応じてIP制限や認可でアクセス制御を強化（今後のオプション）。
...
## 13. 補助可観測性（LangSmith 連携・任意）
- 目的: ツリー/階層表示・詳細メタデータ・比較など高度なトレースを必要とする調査時に、LangSmith のUIを併用。
- 導入（コード変更不要、環境変数のみ）:
  - `LANGCHAIN_TRACING_V2=true`
  - `LANGCHAIN_API_KEY=<your_key>`
  - `LANGCHAIN_PROJECT=<project_name>`
  - （任意）`LANGCHAIN_ENDPOINT=...`
- 参考計画: `plan/done/250905_LangSmith実装計画.md`
